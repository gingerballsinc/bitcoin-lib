...
object DeterministicWallet {
  ...
  case class KeyPath(keyPath: fr.acinq.bitcoin.KeyPath) {
  ...
  object KeyPath {
    val Root = KeyPath(new fr.acinq.bitcoin.KeyPath(""))

    def apply(path: Seq[Long]): KeyPath = new KeyPath(new fr.acinq.bitcoin.KeyPath(path.map(x => Long.box(x)).toList.asJava))
     ...
     */
    def apply(path: String): KeyPath = KeyPath(new fr.acinq.bitcoin.KeyPath(path))
  ...
  case class ExtendedPrivateKey(val priv: fr.acinq.bitcoin.DeterministicWallet.ExtendedPrivateKey) {
  ...
  object ExtendedPrivateKey {
    def apply(secretkeybytes: ByteVector32, chaincode: ByteVector32, depth: Int, derivationPath: KeyPath, parent: Long) = new ExtendedPrivateKey(
      new fr.acinq.bitcoin.DeterministicWallet.ExtendedPrivateKey(secretkeybytes, chaincode, depth, derivationPath.keyPath, parent)
    )

    def decode(input: String, parentPath: KeyPath = KeyPath.Root): (Int, ExtendedPrivateKey) = {
      val p = fr.acinq.bitcoin.DeterministicWallet.ExtendedPrivateKey.decode(input, parentPath.keyPath)
      (p.getFirst, ExtendedPrivateKey(p.getSecond))
    }
  }

  def encode(input: ExtendedPrivateKey, prefix: Int): String = fr.acinq.bitcoin.DeterministicWallet.encode(input.priv, prefix)

  case class ExtendedPublicKey(val pub: fr.acinq.bitcoin.DeterministicWallet.ExtendedPublicKey) {
  ...
  object ExtendedPublicKey {
    def apply(publicKey: ByteVector, chaincode: ByteVector32, depth: Int, derivationPath: KeyPath, parent: Long) = new ExtendedPublicKey(
      new fr.acinq.bitcoin.DeterministicWallet.ExtendedPublicKey(publicKey, chaincode, depth, derivationPath.keyPath, parent)
    )

    def decode(input: String, parentPath: KeyPath = KeyPath.Root): (Int, ExtendedPublicKey) = {
      val p = fr.acinq.bitcoin.DeterministicWallet.ExtendedPublicKey.decode(input, parentPath.keyPath)
      (p.getFirst, ExtendedPublicKey(p.getSecond))
    }
  }

  def encode(input: ExtendedPublicKey, prefix: Int): String = fr.acinq.bitcoin.DeterministicWallet.encode(input.pub, prefix)
   ...
   */
  def generate(seed: ByteVector): ExtendedPrivateKey = ExtendedPrivateKey(fr.acinq.bitcoin.DeterministicWallet.generate(seed))
   ...
   */
  def publicKey(input: ExtendedPrivateKey): ExtendedPublicKey = ExtendedPublicKey(fr.acinq.bitcoin.DeterministicWallet.publicKey(input.priv))
   ...
   */
  def fingerprint(input: ExtendedPublicKey): Long = fr.acinq.bitcoin.DeterministicWallet.fingerprint(input.pub)
   ...
   */
  def fingerprint(input: ExtendedPrivateKey): Long = fr.acinq.bitcoin.DeterministicWallet.fingerprint(input.priv)
   ...
   */
  def derivePrivateKey(parent: ExtendedPrivateKey, index: Long): ExtendedPrivateKey = ExtendedPrivateKey(fr.acinq.bitcoin.DeterministicWallet.derivePrivateKey(parent.priv, index))
   ...
   */
  def derivePublicKey(parent: ExtendedPublicKey, index: Long): ExtendedPublicKey = ExtendedPublicKey(fr.acinq.bitcoin.DeterministicWallet.derivePublicKey(parent.pub, index))
