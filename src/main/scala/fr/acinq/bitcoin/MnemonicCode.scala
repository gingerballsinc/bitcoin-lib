...
object MnemonicCode {
  ...
  private def toBinary(x: ByteVector): List[Boolean] = x.toArray.flatMap(toBinary).toList
  ...
  def validate(mnemonics: Seq[String], wordlist: Seq[String] = englishWordlist): Unit = {
    ...
    val data = databits.grouped(8).map(fromBinary).map(_.toByte).toSeq
    val check = toBinary(Crypto.sha256(ByteVector.view(data.toArray))).take(data.length.toInt / 4)
